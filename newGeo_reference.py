...
def test_time_memory_usage_part1():
    ...
    with rasterio.open(img_path) as unref_raster:
        ...
        with rasterio.open(output_path) as src:
            ...
            print("test1 (with 5 primary dots)")
...
def test_time_memory_usage_part2():
    start_time = time.time()
    ...
    output_path = 'result/georefRaster.tif'
    new_output_path = 'result/new_georefRaster.tif'
    ...
    with rasterio.open(img_path) as unref_raster:
        ...
        with rasterio.open(new_output_path) as src:
            geo_lon, geo_lat = src.xy(8619, 9484)
            print("test2 (with additional dots)")
            print(f'pixel 9484, 8619 --> geo {geo_lat} {geo_lon}')
...
def compress_image(input_file,output_file = 'result\\compressed.jpg', quality=85):
    ...
    if file_size_mb > 79:
        ...
        print(f"Файл '{input_file}' був стиснений. Новий розмір: {os.path.getsize(output_file) / (1024 * 1024):.2f} МБ.")
    else:
        print(f"Файл '{input_file}' має розмір {file_size_mb:.2f} МБ і не потребує стиснення.")
...
test_time_memory_usage_part1()
test_time_memory_usage_part2()
